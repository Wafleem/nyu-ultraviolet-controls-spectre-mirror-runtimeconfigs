/**
  ******************************************************************************
  * @file			w25q128jv.c
  * @brief		Driver for w25q128jv qspi flash with the STM32 HAL
  ******************************************************************************
  * @attention
  *
...
/*Enable quad mode*/
/* Versions ending in IQ have this active by default, unless chip reset */
uint8_t QSPI_Configuration(void) {
	...
	/* Write Volatile Configuration register 3 (DRV1:2 = 00) -- */
	sCommand.Instruction = WRITE_STATUS_REG3_CMD;
	reg &= OUTPUT_DRIVER_100; // DRV1:2 bit
...
uint8_t QSPI_ReadStatus(uint8_t status[3]) {
	...
	if ((ret = HAL_QSPI_Receive(&hqspi, &status[0], HAL_QPSI_TIMEOUT_DEFAULT_VALUE))
			!= HAL_OK) {
		return ret;
	}
	...
	if ((ret = HAL_QSPI_Receive(&hqspi, &status[1], HAL_QPSI_TIMEOUT_DEFAULT_VALUE))
			!= HAL_OK) {
		return ret;
	}
	...
	if ((ret = HAL_QSPI_Receive(&hqspi, &status[2], HAL_QPSI_TIMEOUT_DEFAULT_VALUE))
			!= HAL_OK) {
		return ret;
	}
